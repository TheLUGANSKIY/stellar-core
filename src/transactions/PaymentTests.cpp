...
TEST_CASE("payment", "[tx][payment]")
{
    ...
    auto gateway = root.create("gate");

    // sets up gateway2 account
    auto gateway2 = root.create("gate2");
    ...
    for (auto v : std::vector<int>{2, 3})
    {
        SECTION("protocol version " + std::to_string(v))
        {
            ...
            //{
            //    auto sendToSelf = root.create("send to self");
            ...
            //{
            //    auto issuer = root.create("issuer");
            //    auto source = root.create("source");
            //    auto destination = root.create("destination");
            //    auto seller = root.create("seller");
            ...
            //SECTION("path with bogus offer, bogus offer shows on offers trail") // Subject to change
            //{
            //    auto paymentToReceive = 24 * assetMultiplier;
            //    auto offerSize = paymentToReceive / 2;
            //    auto mm = root.create("mm");
            //    auto source = root.create("source");
            //    auto destination = root.create("destination");
            ...
            //    auto path = std::vector<Asset>{usdCur};
            ...
            //    auto assets = std::deque<Asset>{usdCur, idrCur};
